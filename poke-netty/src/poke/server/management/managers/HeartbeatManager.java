		...
		while (forever) {
			try {
				...
				if (outgoingHB.size() > 0) {
					...
					GeneratedMessage msg1 = null;
					//ElectionManager electionManager = new ElectionManager(nodeId, 1);
					for (HeartbeatData hd : outgoingHB.values()) {
						...
						if (msg == null){
							msg = generateHB();
							//GeneratedMessage msg1 = null;
							msg1 = ElectionManager.getInstance().generateElection();
							logger.info("Sending Election desc as: "+ElectionManager.getInstance().generateElection().getElection().getDesc());
						}	

						try {
							logger.info("@@##sending heartbeat");
							//logger.info("Sending Election desc as: "+generateHB().getElection().getDesc());
							hd.channel.writeAndFlush(msg);
							hd.channel.writeAndFlush(msg1);
